...
It's cleaner and more concise to apply the hook useRouteMatch to your code because it connects your route path and URL to the same function, so when applied you do not need to repeat the same intial segments of the route path, and only write in the parts that will differ.
...
Form validation is the process used to ensure information is properly inputted. It will run a validation processes using a specific expected value to test whether or not the arguments you used work together or produce errors. By arguments I refer to the formSchema and the key you're using to test it. With that information you can test whether a process is successful, and if not you can dictate specific error messages that will appear whenever you do something wrong (e.g. not enough password characters).

3. In 1-2 sentences, define end to end testing.

End to end testing is testing that runs an entire process to check whether a user can accomplish an action. It focuses on the UI and mimics how a use with interact with an app. End to end testing makes up about 10% of testing used. This is different from unit, static, or integration methods which test isolated parts, typos/errors, and the interaction of different segments, respectively. 
